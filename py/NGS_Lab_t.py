...
for each_dir in os.listdir(dir_data):
    ...
    if re.match('%s' % project, each_dir) and os.path.isdir(dir_run) and each_dir not in []:
        print "=>%s" % dir_run
        run_bn = re.match('%s(.+)' % project, each_dir).group(1)
        proj_table[run_bn] = []
        for each_file in os.listdir(dir_run):
            if re.search('\.sam$', each_file):
                basename = re.match('(.+)_S\d+_L\d+', each_file).group(1)
                proj_table[run_bn].append(basename)
        print proj_table[run_bn]

with open('/Users/codeunsolved/Downloads/NGS-Data/DB/%s_lab.csv' % project, 'wb') as w_obj:
